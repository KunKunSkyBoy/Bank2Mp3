...
class MainActivity : AppCompatActivity() {
    ...
    private fun startTitleBreathing() {
        ...
        ObjectAnimator.ofFloat(title, "scaleY", 1f, 1.03f, 1f).apply {
            duration = 4500
            repeatCount = ValueAnimator.INFINITE
            repeatMode = ValueAnimator.REVERSE
            start()
    ...
    private fun applyThemeColors() {
        ...
        val lightBg = 0xFFFFFAF5.toInt()
        val lightSurf = 0xFFF0E4D0.toInt()
        val lightText = 0xFF1A2A40.toInt()
        val lightText2 = 0xFF5A6A80.toInt()
        val lightAccent = 0xFF6C5CE7.toInt()
        b.bridgeStatus.parent.let { (it as? View)?.setBackgroundColor(lightBg) }
        b.tvLog.setBackgroundColor(lightBg)
        b.btnThemeToggle.setBackgroundColor(lightSurf)
        b.tvFile.setTextColor(lightText2)
        b.tvOutputDir.setTextColor(lightText2)
        b.tvLog.setTextColor(lightText)
        b.btnBridgeRefresh.setBackgroundColor(lightAccent)
        b.btnPickFolder.backgroundTintList = android.content.res.ColorStateList.valueOf(lightSurf)
        b.btnPickOutput.backgroundTintList = android.content.res.ColorStateList.valueOf(lightSurf)
        b.btnResetOutput.backgroundTintList = android.content.res.ColorStateList.valueOf(lightSurf)
        b.btnClearLog.backgroundTintList = android.content.res.ColorStateList.valueOf(lightSurf)
    }

    private fun startCardGlow() {
        ValueAnimator.ofFloat(0f, 1f, 0f).apply {
            duration = 4000
            repeatCount = ValueAnimator.INFINITE
            repeatMode = ValueAnimator.RESTART
            addUpdateListener {
                val t = animatedValue as Float
                if (isDarkTheme) {
                    val v = (0x28 + (0x38 - 0x28) * t).toInt()
                    val r = (0x18 + (0x21 - 0x18) * t).toInt()
                    b.bridgeStatus.parent.let { (it as? View)?.setBackgroundColor(Color.rgb(r, v, v + 0x10)) }
                }
            }
            start()
        }
