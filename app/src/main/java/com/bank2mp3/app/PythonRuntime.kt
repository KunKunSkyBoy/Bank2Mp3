...
object PythonRuntime {
    ...
    private fun copyScripts(context: Context) {
        ...
        for (f in listOf("decode.py", "terminal_server.py", "batch_convert.py")) {
            context.assets.open("Bank2Mp3/scripts/$f").use { src ->
                File(scriptsDir, f).outputStream().use { dst -> src.copyTo(dst) }
            }
        }
    }
